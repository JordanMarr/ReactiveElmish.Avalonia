...
module AvaloniaProgram =
    ...
    let mkSimple
        ...
        =
        Program.mkSimple init update (fun _ _ -> ()) |> create bindings
    ...
    let mkProgram
        ...
        (bindings: unit -> Binding<'model, 'msg> list)
        =
        Program.mkProgram init update (fun _ _ -> ()) |> create bindings
    ...
    let startElmishLoop (element: StyledElement) (program: AvaloniaProgram<'model, 'msg>) =
        ...
        let performanceLogger =
            program.LoggerFactory.CreateLogger("Elmish.Avalonia.Performance")
        ...
        let setState model _ =
            ...
            | None ->
                ...
                let uiDispatch msg =
                    Dispatcher.UIThread.Post(fun () -> dispatch msg) |> ignore
                ...
                viewModel <- Some vm
            | Some vm -> IViewModel.updateModel (vm, model)
    ...
    /// Terminates the Elmish program and calls `terminate` when the `predicate` is true.
    let withTermination (predicate: 'msg -> bool) (terminate: 'model -> unit) program =
        { program with
